...
function generate_sub(; p1bar=0, p2bar=0, x1bar=0, x2bar=0, prob=0.5, demand=2)
    ...
    s1 =  Model(solver=MosekSolver(MSK_IPAR_INTPNT_STARTING_POINT=0,MSK_DPAR_INTPNT_TOL_PSAFE=100.0, MSK_IPAR_INTPNT_MAX_ITERATIONS=1000000, MSK_DPAR_INTPNT_TOL_REL_GAP=1e-5, MSK_DPAR_INTPNT_NL_TOL_REL_GAP=1e-5))
 ...
 # Ax+g(y) ⩽0, g2(y)⩽0
    ...
    #0⩽ẋ⩽xᵘᵇλ    0⩽ẏ⩽yᵘᵇλ
    @constraint(s1, b1[k in max_djc], dot_p1[k] <= 4*dot_λ[k])
    @constraint(s1, b2[k in max_djc], dot_p2[k] <= 2*dot_λ[k])
