...
function generate_master(; mult_x=[], mult_y=[], g=[], iter=[] )
	m = Model(solver=CplexSolver())
 	...
 	#Benders cuts
 	@constraint(m, b1[s in scenarios, it in iter], η>= g[it][s] + sum(mult_x[it][s][i] * x[i] + mult_y[it][s][i]*y[i] for i in rectangles))
