...
include("cnf.jl")
 function generate_fullspace()
 	...
 	@variable(m, ff)
 	@constraint(m, ff == sum(C[i,j] * (dely[i,j] + delx[i,j]) for i in rectangles for j in rectangles if i<j))
 ...
 ff = getvalue(getindex(m, :ff))
 x = getvalue(getindex(m, :x))
 ...
 total_obj = ff + sum(obj_record[s] for s in scenarios)
 println(total_obj)
 println(obj_record)
