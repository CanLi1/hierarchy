...
prob = [0.5 0.5]
demand =[1.5 2]
function generate_node1(; p1bar=0, p2bar=0, x1bar=0, x2bar=0, prob=0.5, demand=2)
    ...
    ϵ=1e-6 
    @variable(s1, p1>=0)
    @variable(s1, p2>=1.0346717044982208)
    @variable(s1, x1)
    @variable(s1, x2)
 ...
 # Ax+g(y) ⩽0, g2(y)⩽0
    ...
    @constraint(s1, bnode1[k in max_djc], dot_p2[k] >= 1.0346717044982208*dot_λ[k])
