function generate_ubsub(; p1bar=0, p2bar=0, x1bar=0, x2bar=0, prob=0.5, demand=2)
    # s1 =  Model(solver=PajaritoSolver(rel_gap=0.00001, mip_solver=GurobiSolver(Threads=1), cont_solver=IpoptSolver()))
    s1 = Model(solver=KnitroSolver(KTR_PARAM_OUTLEV=0))
    @variable(s1, p1>=0)
    @variable(s1, p2>=0)
    @variable(s1, x1)
    @variable(s1, x2)
