using KNITRO
function generate_knitro_sub(; p1bar=0, p2bar=0, x1bar=0, x2bar=0, prob=0.5, demand=2, ub=[4 2], lb=[0 0])
    # s1 =  Model(solver=MosekSolver(MSK_IPAR_NUM_THREADS=1))
    s1 =  Model(solver=KnitroSolver(MSK_DPAR_OPTIMIZER_MAX_TIME=50.0, MSK_IPAR_NUM_THREADS=1, MSK_IPAR_INTPNT_MAX_ITERATIONS=1000000, MSK_DPAR_INTPNT_TOL_REL_GAP=1e-5, MSK_DPAR_INTPNT_NL_TOL_REL_GAP=1e-5))
