...
function generate_knitro_sub(; djc=[[1],[2],[3],[4],[5],[6],[7],[8],[9]], Q=zeros(5), prob=0.0)
    # s1 = Model(solver=KnitroSolver())
    s1 = Model(solver=IpoptSolver())
    # s1 = Model(solver=MosekSolver( MSK_IPAR_NUM_THREADS=0, MSK_IPAR_INTPNT_MAX_ITERATIONS=1000000))
    #MSK_IPAR_LOG=0,
    #, KTR_PARAM_OUTLEV=1
