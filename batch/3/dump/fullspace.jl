...
function generate_fullspace()
	...
	@NLconstraint(m, e9, firststagecost>= sum(alpha[j] * exp(n[j] + beta[j] * v[j]) for j in stages))
	@NLconstraint(m, e10, secondstagecost>=sum(prob[w] * ( sum( lambda[j] * exp(ns[j,w]) for j in stages)+ delta*L[w]) for w in scenarios ))

	@NLobjective(m, Min, firststagecost + secondstagecost )
	return m 
end

m = generate_fullspace()
solve(m)
println(getvalue(getindex(m, :v)))
