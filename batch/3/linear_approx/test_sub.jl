...
function generate_fullspace()
	# m = Model(solver=PajaritoSolver(rel_gap=0.001, timeout=10000, mip_solver=CplexSolver(), cont_solver=IpoptSolver()))
	...
	@constraint(m, e7[int in intp, ii in products, w in scenarios], sum(Q[i,w] * tl_b[i,w]  for i in products) <= H + L[w])
	@constraint(m, e8[j in stages, w in scenarios], sum(ys[int,j,w] for int in integer) == 1)

	@constraint(m, e9[int in intp, jj in stages], firststagecost>= sum(alpha[j] * n_v[j] for j in stages))
...
for s in scenarios
	...
	for j in stages
		JuMP.setRHS(getindex(relax_sub[s], :t2)[j], v[j])
		JuMP.setRHS(getindex(relax_sub[s], :t3)[j], n[j])		
		for int in integer
			JuMP.setRHS(getindex(relax_sub[s], :t1)[int, j], yf[int, j])
		end
	end
	solve(relax_sub[s])
