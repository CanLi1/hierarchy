...
@everywhere include("knitro_sub.jl")
...
obj_master = []
a = now()
while relax_UB > relax_LB *1.0005
    c = now()
    solve(m)
    d = now()
...
while true
    ...
    while true
        ...
        sub_time = sub_time + d - c
        for s in scenarios
            if results[s][:status] == :error
                sub_problem[s] = generate_knitro_sub(djc=djc_scenarios[s], Q=Q[:,s], prob =prob[s])
                for j in stages
                    setvalue(getindex(sub_problem[s], :nbar)[j], nbarr[j])
                    setvalue(getindex(sub_problem[s], :vbar)[j], vbarr[j])
                    for int in integer
                ...
                results[s]=psolve(sub_problem[s])
                sub_problem[s] = generate_sub(djc=djc_scenarios[s], Q=Q[:,s], prob =prob[s])
            end   
        end           
        ...
        if length(mult_yf) > temp_length + 30
            break
        end
