...
basephi(c,j) "penalty cost for not satisfying demand from customer c for chemical j"
phi(c,j,w)
QEU(p,i)
PUU /75/
FUU /75/
  ...
  loop(sub1,
    loop(sub2,
      loop(sub3,
            ...
            loop(w,
              if(ord(w) eq num + 1,
D('c1',j,w)= baseD('c1', j)*(1+(ord(sub0)-2)*0.3);
D('c2',j,w)= baseD('c2', j)*(1+(ord(sub1)-2)*0.3);
phi(c,'j3',w) = basephi(c,'j3') * (1+(ord(sub2)-2)*0.3);
