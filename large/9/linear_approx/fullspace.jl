...
function generate_fullspace()
	...
	@constraint(m, e8[p in plant, i in process, j in chemical, jj in chemical, w in scenarios, s in scheme, int in intp; (i,s,j) in Lbar && (i,s) in PS && (i,s,jj) in JM], log(1.0+intWW[int]) + 1/(1+intWW[int])*(WW[p,i,j,s,w]-intWW[int]) >= mu[i,s,j] * WW[p,i,jj,s,w])
