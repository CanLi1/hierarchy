...
function generate_nlprelax(;  demand=zeros(length(customer),length(chemical)), price=zeros(length(customer),length(chemical)),  prob=0.0, Qbar=zeros(length(plant), length(process)), xbar=zeros(length(plant), length(process)))
	...
	@constraint(s1, e8[p in plant, i in process, j in chemical, jj in chemical, s in scheme, int in intp; (i,s,j) in Lbar && (i,s) in PS && (i,s,jj) in JM], log(1.0+intWW[int]) + 1/(1.0+intWW[int])*(WW[p,i,j,s]-intWW[int])>= mu[i,s,j] * WW[p,i,jj,s])
