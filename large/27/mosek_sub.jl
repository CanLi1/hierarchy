...
function generate_mosek_sub(; djc=[[1],[2],[3],[4],[5],[6],[7],[8]], demand=zeros(2,6), price=zeros(2,6),  prob=0.0)
	s1 = Model(solver=MosekSolver(MSK_IPAR_NUM_THREADS=0, MSK_IPAR_LOG=0, MSK_IPAR_INTPNT_MAX_ITERATIONS=1000000, MSK_DPAR_INTPNT_TOL_REL_GAP=1e-5, MSK_DPAR_INTPNT_NL_TOL_REL_GAP=1e-5))
	# s1 = Model(solver=IpoptSolver(tol=1e-6, constr_viol_tol=1e-3, compl_inf_tol=1e-3))
    # s1 = Model(solver=KnitroSolver(KTR_PARAM_FEASTOL=1e-5, KTR_PARAM_OPTTOL=1e-5,KTR_PARAM_INFEASTOL=1e-10))
	ϵ=1e-5
	...
	@NLconstraint(s1, c8[p in plant, i in process, j in chemical, jj in chemical, s in scheme, d in disjunction, k in max_djc; (i,s,j) in Lbar && (i,s) in PS && (i,s,jj) in JM && k<= 2^(length(djc[d]))], ( (1-ϵ)* dot_λ[d,k]  + ϵ)*log(1.0+dot_WW[p,i,j,s,d,k]/((1-ϵ)*dot_λ[d,k] + ϵ)) >= mu[i,s,j] * dot_WW[p,i,jj,s,d,k])
