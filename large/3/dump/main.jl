...
while UB > LB + 1e-3
    ...
    for s in scenarios
        a = now()
    	push!(sub_problem, generate_sub(demand=D[1:2, 1:6, s], price=phi, prob=prob[s], xbar=xbarr, Qbar=QEbarr))
    ...
    if length(mult_QE)> 10 #||( length(mult_QE)>1 && obj_master[length(mult_QE)] - obj_master[length(mult_QE)-1] <1e-3)
    	break
    end
end
