...
@everywhere include("linear_sub.jl")
...
for s in scenarios
    push!(sub_problem, generate_nlprelax(demand=D[1:4, 1:6, s], price=phi, prob=prob[s]))
end
...
while relax_UB > relax_LB + 1e-1
    ...
    for s in scenarios
        ...
        for p in plant
            for i in process
                JuMP.setRHS(getindex(sub_problem[s], :t1)[p,i], QEbarr[p,i])
                JuMP.setRHS(getindex(sub_problem[s], :t2)[p,i], xbarr[p,i])
            end
        end
...
while UB > LB * 1.001
    ...
    sub_problem=[]
    for s in scenarios
        push!(sub_problem, generate_sub(djc=djc_scenarios[s], demand=D[1:4, 1:6, s], price=phi, prob=prob[s]))
    end
    m = generate_master(mult_QE=mult_QE, mult_x=mult_x, g=g, iter=1:length(mult_QE))
    ...
    for s in scenarios
        ...
        for p in plant
            for i in process
                JuMP.setRHS(getindex(sub_problem[s], :t1)[p,i], QEbar_for_ub[p,i])
                JuMP.setRHS(getindex(sub_problem[s], :t2)[p,i], xbar_for_ub[p,i])
            end
        end
        c = now()
    ...
    ub_sub_problem = []
    for s in scenarios
        push!(ub_sub_problem, generate_ubsub(demand=D[1:4, 1:6, s], price=phi, prob=prob[s], Qbar=QEbar_for_ub, xbar=xbar_for_ub))
    end
